...
if __name__ == '__main__':
    ...
    for i in np.where(excluded)[0]:
        ...
        # bring Ne back to linear scale
        value[2] = 10**value[2]
        interpolated_structure[2] = 10**(interpolated_structure[2])
        
        # write to file
        ma_interpol = model_atmosphere()
        ma_interpol.id = f"{name_exc}_interpol"
        ...
        ma_orig.ne = 10**(value[2])
        ma_orig.vmac = np.zeros(len(ma_interpol.depth_scale))
        ma_orig.vturb = value[3]

        ma_orig.write( f"{save_interpol_dir}/atmos.{ma_orig.id}", format='ts')
